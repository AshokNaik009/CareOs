...
export function createApp({ origin = 'http://localhost:3000', clientId = '', clientSecret = '', production = false, pageFile, fetchImpl = fetch, now = Date.now, twilio = {}, sampleData = false } = {}) {
  const app = express();
  ...
  const alerts = createAlertService({ sessions, whoop, origin, twilio, fetchImpl, now });
  ...
  router.get('/api/session', (req, res) => res.json({ configured, connected: Boolean(req.session?.tokens || req.session?.sample), sample: Boolean(req.session?.sample), sampleAvailable: sampleData }));
  router.get('/auth/sample', (req, res) => {
    if (!sampleData) throw new AppError(404, 'Endpoint not found.');
    clearExpired();
    if (sessions.size >= 1000) throw new AppError(503, 'Please try again later.');
    if (req.sessionId) sessions.delete(req.sessionId);
    const id = randomBytes(32).toString('base64url');
    sessions.set(id, { sample: true, expiresAt: now() + SESSION_AGE });
  ...
  async function report(req, res) {
    if (!req.session?.tokens && !req.session?.sample) throw new AppError(401, 'Connect your WHOOP account to see your daily analysis.');
    ...
    const data = req.session.sample ? createSampleData(now()) : await whoop.data(req.session);
    ...
    if (!req.session.sample) void alerts.check(req.session);
    res.json(req.path === '/api/analysis' ? result.analysis : { ...result, timeZone, fetched_at: data.fetchedAt, sample: Boolean(req.session.sample) });
  }
