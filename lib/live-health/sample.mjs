// Mock REST mode (LIVE_HEALTH_SAMPLE_DATA=true by default). Produces WHOOP-shaped records so the normal
...
export function createSampleData(now = Date.now()) {
  ...
  for (let i = 30; i >= 0; i--) {
    ...
    const updated = new Date(Math.min(now, wake + 5 * 60000)).toISOString();
    sleeps.push({
