...
import { createAlertService, verifyWhoopSignature, verifyRetellSignature } from './alerts.mjs';

const SESSION_AGE = 86400000;
const cookieName = 'rafeeq_whoop_session';
const basePath = '/live-health';

export function createApp({ origin = 'http://localhost:3000', clientId = '', clientSecret = '', production = false, pageFile, fetchImpl = fetch, now = Date.now, retell = {} } = {}) {
  const app = express();
  ...
  const alerts = createAlertService({ sessions, whoop, retell, fetchImpl, now });
  ...
  const monitor = setInterval(() => { for (const session of sessions.values()) void alerts.check(session); }, 300000);
  cleanup.unref();
  monitor.unref();
  ...
  function webhook(req, res) {
    ...
    const provider = req.params.provider ?? 'retell';
    if (!['whoop', 'retell'].includes(provider)) throw new AppError(404, 'Webhook not found.');
    const verified = provider === 'whoop'
      ? verifyWhoopSignature(body, req.get('X-WHOOP-Signature-Timestamp'), req.get('X-WHOOP-Signature'), clientSecret, now())
      : verifyRetellSignature(body, req.get('X-Retell-Signature'), retell.webhookKey || retell.apiKey, now());
    if (!verified) throw new AppError(401, 'Invalid webhook signature.');
    let payload;
    try { payload = JSON.parse(body); } catch { throw new AppError(400, 'Invalid JSON body.'); }
    if (provider === 'whoop') alerts.handleWhoop(payload);
    else alerts.handleRetell(payload);
    res.sendStatus(204);
  }
  app.post('/retell/webhook', rawWebhook, webhook);
  app.use(basePath, router);
  router.post('/webhooks/:provider', rawWebhook, webhook);
