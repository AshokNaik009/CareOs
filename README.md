...
- **Frontend:** React 19 + Vite 6 (JavaScript/JSX), built as four pages: `/`, `/patient.html`, `/provider.html`, `/live-health`.
...
Live Health has Node unit/API tests and Playwright Chromium tests, including regression checks for the existing demo routes. External WHOOP requests and Bluetooth devices are mocked; the browser-test server explicitly disables all AI and WHOOP credentials. Use Node 22.12+ for the test toolchain. There is no linter configured.
